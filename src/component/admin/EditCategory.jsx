...
function EditCategory({close ,fetchData ,data:categoryData}) {
     const [data, setData] = useState({
      ...
      const handleSubmit = async (e) => {
        ...
        const response = await axios [SummaryApi.updateCategory.method](SummaryApi.updateCategory.url ,{name :data.name ,image:data.image ,categoryId: data._id},{ withCredentials: true } )
    ...
    <div className="fixed top-0 bottom-0 left-0 right-0 bg-[#00000075] flex justify-center items-center z-50 ">
          <div className="w-[90%] md:w-[70%] lg:w-[60%] p-[20px] bg-white rounded-lg">
            ...
            <form className="my-4 flex flex-col gap-3" onSubmit={handleSubmit}>
              ...
              >
                {isUploading ? "Processing..." : "Update Category"}
              </button>
            </form>
          </div>
        </div>
